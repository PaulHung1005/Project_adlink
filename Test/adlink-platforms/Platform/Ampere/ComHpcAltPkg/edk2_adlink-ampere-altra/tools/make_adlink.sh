...
OEM_COMMON_DIR=$WORKSPACE/adlink-platforms/Platform/Ampere/ComHpcAltPkg/edk2_adlink-ampere-altra
...
echo "$(pwd)"
if [ -d "$(pwd)/adlink-platforms/Platform/Ampere/ComHpcAltPkg/Override" ]; then
  echo "override exists"
  . compare.sh
  . copy.sh
else
  echo "override does not exists"
fi
