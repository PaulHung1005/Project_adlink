...
{
  UINT8 MaxWidth;
  DEBUG ((DEBUG_INFO,"[adlink]In SetRootComplexBifurcation\n"));
  DEBUG ((DEBUG_INFO,"[adlink]RPStart = %d,RootComplex->ID= %d \n",RPStart,RootComplex->ID));
 /* if (RPStart != PcieController0 && RPStart != PcieController4) {
      DEBUG ((DEBUG_INFO,"Returned 1\n"));
    return;
  }

  if (RootComplex->Type != RootComplexTypeB && RPStart == PcieController4) {
      DEBUG ((DEBUG_INFO,"Returned 2\n"));
    return;
  }*/
  ...
  if (RootComplex->Type == RootComplexTypeA && RootComplex->Pcie[RPStart].MaxWidth == LINK_WIDTH_X16) {
    ...
    DEBUG ((DEBUG_INFO,"[adlink]In Max width = x16\n"));
  }
  ...
  if (RootComplex->Type == RootComplexTypeB && RootComplex->Pcie[RPStart].MaxWidth == LINK_WIDTH_X8) {
    ...
    DEBUG ((DEBUG_INFO,"[adlink]In Max width = x8\n"));
  }
  ...
  case DevMapMode2:
    ...
    DEBUG ((DEBUG_INFO,"[adlink]In case DevMapMode2\n"));
    break;
  ...
  case DevMapMode3:
    ...
    DEBUG ((DEBUG_INFO,"[adlink]In case DevMapMode3\n"));
    break;
  ...
  case DevMapMode4:
    ...
    RootComplex->Pcie[RPStart + 3].MaxWidth = PCIE_GET_MAX_WIDTH (RootComplex->Pcie[RPStart + 3], MaxWidth);
    RootComplex->Pcie[RPStart + 3].Active = TRUE;
    DEBUG ((DEBUG_INFO,"[adlink]In case DevMapMode4\n"));
    break;
  ...
  default:
    ...
    RootComplex->Pcie[RPStart + 3].Active = FALSE;
    DEBUG ((DEBUG_INFO,"[adlink]In case DevMapMode default\n"));
    break;
  }
}
  ...
  )
{
  ...
  if (RootComplex->Type == RootComplexTypeA) {
    if (RootComplex->ID < MaxPcieControllerOfRootComplexA) {
      ...
      DEBUG ((DEBUG_INFO,"[adlink]In RootComplex->Type == RootComplexTypeA\n"));
    } else {
      ...
      DEBUG ((DEBUG_INFO,"[adlink]In RootComplex->Type == Else\n"));
    }
  } else {
    //
    ...
    DEBUG ((DEBUG_INFO,"[adlink]In Entering RCB with 2 params\n"));
  }
  ...
  for (RPIndex = PcieController0; RPIndex < Controller; RPIndex++) {
    ...
    case 3:
    case 4:
        DEBUG ((DEBUG_INFO,"[adlink]Case 4\n"));
    ...
    default:
      ...
      DEBUG ((DEBUG_INFO,"[adlink]Case default4\n"));
      break;
    }
  }
  ...
  if (Value == AUTO_BIFURCATION_SETTING_VALUE) {
   ...
   // RootComplex->Pcie[PcieController0].MaxGen = LINK_SPEED_GEN3;
    ...
    DEBUG ((DEBUG_INFO,"[adlink]Auto bifurcation value 4\n"));
  }

  if (RootComplex->Type == RootComplexTypeB) {
    NvParamOffset += NV_PARAM_ENTRYSIZE;
    Status = NVParamGet (NvParamOffset, NV_PERM_ALL, &Value);
    DEBUG ((DEBUG_INFO,"[adlink]RCB set offset 4\n"));
    if (EFI_ERROR (Status)) {
      Value = 0;
    }
    ...
    for (RPIndex = MaxPcieControllerOfRootComplexA; RPIndex < MaxPcieController; RPIndex++) {
      ...
      case 4:
		...
		//><ADLINK-MS20232710>//
        RootComplex->Pcie[RPIndex].Active = TRUE;
        DEBUG ((DEBUG_INFO,"[adlink]MaxWidth = 1 << Width\n"));
        break;
      ...
      default:
		...
		//><ADLINK-MS20232710>//
        RootComplex->Pcie[RPIndex].Active = FALSE;
        DEBUG ((DEBUG_INFO,"[adlink]MaxWidth = LINK_WIDTH_NONE\n"));
        break;
      }
    }
  }
  ...
  if (RootComplex->Socket == 0) {
    if (RootComplex->Type == RootComplexTypeA) {
      if (RootComplex->ID < MaxRootComplexA) {
        if (IsAc01Processor()) {
            DEBUG ((DEBUG_INFO,"[adlink]NV_SI_RO_BOARD_S0_RCA0_TXRX_G3PRESET 4\n"));
          ...
          DEBUG ((DEBUG_INFO,"[adlink]NV_SI_RO_BOARD_MQ_S0_RCA0_TXRX_G3PRESET 4\n"));
        }
      } else {
        if (IsAc01Processor()) {
            DEBUG ((DEBUG_INFO,"[adlink]NV_SI_RO_BOARD_S0_RCA4_TXRX_G3PRESET 4\n"));
          NvParamOffset = NV_SI_RO_BOARD_S0_RCA4_TXRX_G3PRESET + (RootComplex->ID - MaxRootComplexA) * NV_PARAM_ENTRYSIZE;
        } else {
            DEBUG ((DEBUG_INFO,"[adlink]NV_SI_RO_BOARD_MQ_S0_RCA4_TXRX_G3PRESET 4\n"));
  ...
  if (Hob != NULL) {
    ...
    if ((PlatformHob->ScuProductId[0] & 0xff) == 0x01) {
                  ...
                  || PlatformHob->AHBCId[1] == 0x21100)))
      {
        ...
        DEBUG ((DEBUG_INFO, "[adlink]RootComplex[%d]: Flags 0x%x\n", RootComplex->ID, RootComplex->Flags));
      }
    }
  }
