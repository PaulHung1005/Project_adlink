...
// For IsNvmeDevicePath and IsUsbDevicePath
EFI_DEVICE_PATH_PROTOCOL *Node;

// For SortBootOptions
EFI_BOOT_MANAGER_LOAD_OPTION Temp;
UINTN CurrentIndex;
UINTN CompareIndex;
UINTN PriorityCurrent;
UINTN PriorityCompare;

// For UpdateBootOrder
UINT16 *NewBootOrder;
UINTN Index;

// For PrioritizeNvmeBootOptions
...
/**
  Check whether the given device path contains a USB node.

  This function traverses the device path and returns TRUE if any node is of
  type MESSAGING_DEVICE_PATH and subtype MSG_USB_DP or MSG_USB_CLASS_DP,
  indicating a USB device.

  @param[in]  DevicePath   A pointer to an EFI_DEVICE_PATH_PROTOCOL.

  @retval TRUE   Device path contains a USB node.
  @retval FALSE  No USB node in the path.
**/
BOOLEAN
IsUsbDevicePath (
  IN EFI_DEVICE_PATH_PROTOCOL *DevicePath
...
/**
  Check whether the given device path contains an NVMe namespace node.

  This function traverses the given device path and returns TRUE if any of its nodes
  match the Messaging Device Path with subtype MSG_NVME_NAMESPACE_DP. This indicates
  that the device path is associated with an NVMe device.

  @param[in]  DevicePath   A pointer to an EFI_DEVICE_PATH_PROTOCOL representing the device path.

  @retval TRUE   The device path contains an NVMe device node.
  @retval FALSE  The device path does not reference an NVMe device.
**/
...
/**
  Sort the boot options to prioritize USB devices first, then NVMe devices,
  followed by all other devices.

  This function iterates through the given array of boot options and reorders
  the entries in-place based on the device type priority:
    - USB devices are given highest priority and appear first.
    - NVMe devices follow USB devices.
    - All other device types are placed after USB and NVMe entries.

  The function uses helper functions IsUsbDevicePath() and IsNvmeDevicePath()
  to determine the type of device associated with each boot option's device path.

  @param[in, out] BootOptions   Pointer to an array of EFI_BOOT_MANAGER_LOAD_OPTION
                                structures representing boot options. The array will
                                be modified in-place to reflect the new order.
  @param[in]      OptionCount   Number of entries in the BootOptions array.
...
{
  for (CurrentIndex = 0; CurrentIndex < OptionCount; CurrentIndex++) {
    for (CompareIndex = CurrentIndex + 1; CompareIndex < OptionCount; CompareIndex++) {
      PriorityCurrent = 2;
      PriorityCompare = 2;

      if (IsUsbDevicePath(BootOptions[CurrentIndex].FilePath)) {
        PriorityCurrent = 0;
      } else if (IsNvmeDevicePath(BootOptions[CurrentIndex].FilePath)) {
        PriorityCurrent = 1;
      }

      if (IsUsbDevicePath(BootOptions[CompareIndex].FilePath)) {
        PriorityCompare = 0;
      } else if (IsNvmeDevicePath(BootOptions[CompareIndex].FilePath)) {
        PriorityCompare = 1;
      }

      if (PriorityCurrent > PriorityCompare) {
        Temp = BootOptions[CurrentIndex];
        BootOptions[CurrentIndex] = BootOptions[CompareIndex];
        BootOptions[CompareIndex] = Temp;
      }
    }
  }
}
...
/**
  Update the UEFI BootOrder variable based on a sorted list of boot options.

  This function constructs a new BootOrder array using the OptionNumber field
  of each boot option in the input list, and updates the BootOrder variable in
  UEFI NVRAM. This effectively redefines the order in which the firmware attempts
  to boot from available devices.

  @param[in] BootOptions   Pointer to an array of sorted boot options.
  @param[in] OptionCount   Number of boot options in the array.
**/
...
{
  NewBootOrder = AllocateZeroPool(OptionCount * sizeof(UINT16));
  if (NewBootOrder == NULL) {
    return;
  }

  for (Index = 0; Index < OptionCount; Index++) {
    NewBootOrder[Index] = BootOptions[Index].OptionNumber;
  }
...
/**
  Prioritize NVMe boot options by updating the BootOrder accordingly.

  This is the main orchestration routine that:
  1. Retrieves all current boot options.
  2. Sorts them to place NVMe devices at the front.
  3. Updates the UEFI BootOrder variable to reflect the new order.
  4. Frees all allocated memory after processing.

  This ensures NVMe boot entries are attempted before non-NVMe devices
  during system boot.

**/

VOID
PrioritizeNvmeBootOptions (
  VOID
  )
{
  BootOptions = EfiBootManagerGetLoadOptions(&OptionCount, LoadOptionTypeBoot);
  if (BootOptions == NULL || OptionCount == 0) {
    return;
  }

  SortBootOptions(BootOptions, OptionCount);
  UpdateBootOrder(BootOptions, OptionCount);
