...
UINTN CurrentIndex;
UINTN CompareIndex;
BOOLEAN CurrentIsNvme;
BOOLEAN CompareIsNvme;
...
/**
  Sort the boot options to prioritize NVMe devices first.

  This function iterates through the given boot option array and reorders
  the entries in-place. NVMe boot options are moved toward the beginning of
  the array, ensuring that they are prioritized in the final boot order.

  @param[in, out] BootOptions   Pointer to an array of boot options to be sorted.
  ...
  )
{
  for (CurrentIndex = 0; CurrentIndex < OptionCount; CurrentIndex++) {
    for (CompareIndex = CurrentIndex + 1; CompareIndex < OptionCount; CompareIndex++) {
      CurrentIsNvme  = IsNvmeDevicePath(BootOptions[CurrentIndex].FilePath);
      CompareIsNvme = IsNvmeDevicePath(BootOptions[CompareIndex].FilePath);
      if (!CurrentIsNvme && CompareIsNvme) {
